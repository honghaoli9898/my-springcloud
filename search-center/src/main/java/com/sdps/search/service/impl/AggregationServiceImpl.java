...
import com.sdps.search.model.AggItemVo;
import com.sdps.search.service.IAggregationService;
...
public class AggregationServiceImpl implements IAggregationService {
	...
	/**
	 * 访问统计聚合查询，需要es里面提供以下结构的数据 { ip, //访问ip browser, //浏览器 operatingSystem,
	 * //操作系统 timestamp //日志时间 }
	 *
	 * @param indexName
	 *            索引名
	 * @param routing
	 *            es的路由
	 * @return 返回结果样例如下 { "currDate_uv": 219, "currDate_pv": 2730,
	 *         "currWeek_pv": 10309, "currHour_uv": 20, "browser_datas": [ {
	 *         "name": "CHROME", "value": 7416 }, { "name": "SAFARI", "value":
	 *         232 }, ... ], "browser_legendData": [ "CHROME", "SAFARI", ... ],
	 *         "operatingSystem_datas": [ { "name": "WINDOWS_10", "value": 6123
	 *         }, { "name": "MAC_OS_X", "value": 1455 }, ... ], "currMonth_pv":
	 *         10311, "statWeek_uv": [ 487, 219, ... ],
	 *         "operatingSystem_legendData": [ "WINDOWS_10", "MAC_OS_X", ... ],
	 *         "statWeek_items": [ "2019-05-08", "2019-05-09", ... ],
	 *         "statWeek_pv": [ 7567, 2730 ... ] }
	 */
	@Override
		...
		searchSourceBuilder
				...
				// 聚合查询当天的数据
						AggregationBuilders
								...
								.field("timestamp")
								.addRange(
										zonedDateTime.withHour(0).withMinute(0)
				...
				// 聚合查询7天内的数据
						AggregationBuilders
								...
								.field("timestamp")
								.addRange(zonedDateTime.minusDays(1),
										zonedDateTime)
								.subAggregation(
								// 聚合并且按小时分组查询当天内的数据
										AggregationBuilders
												...
												// 时区相差8小时
												.timeZone(
														ZoneId.of(CommonConstant.TIME_ZONE_GMT8))
												.minDocCount(0L)
												.extendedBounds(
														new LongBounds(
																curDateTime
				...
				// 聚合查询7天内的数据
						AggregationBuilders
								.dateRange("currWeek")
								.field("timestamp")
								.addRange(zonedDateTime.minusDays(7),
										zonedDateTime)
								.subAggregation(
								// 聚合并且按日期分组查询7天内的数据
										AggregationBuilders
														...
														new LongBounds(
																localDate
																		.minusDays(
																				6)
																		.format(DateTimeFormatter
																				.ofPattern(CommonConstant.DATE_FORMAT)),
																localDate
																		.format(DateTimeFormatter
																				.ofPattern(CommonConstant.DATE_FORMAT))))
												.subAggregation(
				...
				// 聚合查询30天内的数据
						AggregationBuilders
								.dateRange("currMonth")
								.field("timestamp")
								.addRange(zonedDateTime.minusDays(30),
										zonedDateTime))
				.aggregation(
				// 聚合查询浏览器的数据
						AggregationBuilders.terms("browser").field(
								"browser.keyword"))
				.aggregation(
				// 聚合查询操作系统的数据
						AggregationBuilders.terms("operatingSystem").field(
				...
				// 聚合查询1小时内的数据
						AggregationBuilders
								.dateRange("currHour")
								.field("timestamp")
								.addRange(zonedDateTime.minusHours(1),
										zonedDateTime)
								.subAggregation(
										AggregationBuilders.cardinality("uv")
												.field("ip.keyword"))).size(0);
	 ...
	 * 赋值当天统计
	 */
	private void setCurrDate(Map<String, Object> result,
	 ...
	 * 赋值周统计
	 */
	private void setCurr24Hour(Map<String, Object> result,
			Aggregations aggregations) {
	 ...
	 * 赋值周统计
	 */
	private void setCurrWeek(Map<String, Object> result,
			Aggregations aggregations) {
	 ...
	 * 赋值月统计
	 */
	private void setCurrMonth(Map<String, Object> result,
			Aggregations aggregations) {
	 ...
	 * 赋值单字段统计
	 */
	private void setTermsData(Map<String, Object> result,
			Aggregations aggregations, String key) {
	 ...
	 * 赋值周趋势统计
	 */
	private void setStatWeek(Map<String, Object> result,
			Aggregations aggregations) {
	 ...
	 * 赋值小时内统计-当前在线数
	 */
	private void setCurrHour(Map<String, Object> result,
			Aggregations aggregations) {
	 ...
	 * 赋值天趋势统计
	 */
	private void setStatDate(Map<String, Object> result,
			Aggregations aggregations) {
