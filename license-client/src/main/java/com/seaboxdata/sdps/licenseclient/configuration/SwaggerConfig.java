...
public class SwaggerConfig{
    ...
    private static final String DEFAULT_PATH = "/api/applicationmgr";
    ...
    public Docket createRestApi() {
        ...
        userIdPar.name("x-cas-uid").description("用户ID").modelRef(new ModelRef("string")).
                parameterType("header").required(false).defaultValue("1").build();
        pars.add(userIdPar.build());

        return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).select()
                .apis(RequestHandlerSelectors.basePackage("com.seaboxdata.sais.licenseclient"))
                .paths(PathSelectors.any()).build().globalOperationParameters(pars);
    }

    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("应用中心-API").description("应用中心后端接口")
                .version("1.0").build();
    }
