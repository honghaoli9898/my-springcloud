...
@Slf4j
@Service(value = "SHBASE")
public class ShbaseServiceImpl implements ServerLogin {
    ...
    public static void main(String[] args) {
        ...
        sdpsServerInfo.setHost("10.1.3.113");
        sdpsServerInfo.setPort("9527");
        sdpsServerInfo.setPasswd("admin");
        sdpsServerInfo.setUser("admin123");
        ...
        if (HttpStatus.OK.value() != responseEntity.getStatusCodeValue()) {
            log.error("调用登录接口失败,url={}", url);
            throw new BusinessException("调用shbase查询接口失败");
        }
        log.info("----------header={}", responseEntity.getHeaders());
        List<String> cookies = responseEntity.getHeaders().get("Set-Cookie");
        if (CollUtil.isEmpty(cookies)) {
