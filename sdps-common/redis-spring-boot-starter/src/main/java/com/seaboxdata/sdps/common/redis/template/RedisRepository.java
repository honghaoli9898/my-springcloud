package com.seaboxdata.sdps.common.redis.template;
...
public class RedisRepository {
	 ...
	 *            过期时间单位
	 */
	public void setExpire(final String key, final Object value,
		...
		redisTemplate.execute(new RedisCallback<Object>() {
			...
			public void potentiallyUsePsetEx(RedisConnection connection) {
				if (!TimeUnit.MILLISECONDS.equals(timeUnit)
						|| !failsafeInvokePsetEx(connection)) {
					connection.setEx(rawKey,
							TimeoutUtils.toSeconds(time, timeUnit), rawValue);
				}
			}
	 ...
	 *            过期时间(单位秒)
	 */
	public void setExpire(final String[] keys, final Object[] values,
			final long time) {
		for (int i = 0; i < keys.length; i++) {
			redisTemplate.opsForValue().set(keys[i], values[i], time,
					TimeUnit.SECONDS);
		}
	}
	...
	public Object get(final String key, RedisSerializer<Object> valueSerializer) {
		...
		return redisTemplate.execute(
				connection -> deserializeValue(connection.get(rawKey),
	 ...
	 * @return Long
	 */
	public Long delCollection(Collection<String> keyCollection) {
	...
	public List<Object> getList(String key, int start, int end,
		...
		return redisTemplate
				.execute(
						connection -> deserializeValues(
								connection.lRange(rawKey, start, end),
								valueSerializer), true);
	}
