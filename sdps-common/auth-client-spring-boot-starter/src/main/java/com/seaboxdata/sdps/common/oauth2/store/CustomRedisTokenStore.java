package com.seaboxdata.sdps.common.oauth2.store;
...
/**
 * 优化自Spring Security的RedisTokenStore 1. 支持redis所有集群模式包括cluster模式 2.
 * 使用pipeline减少连接次数，提升性能 3. 自动续签token（可配置是否开启）
 *
 * @author zlt
 * @date 2019/7/7
 */
public class CustomRedisTokenStore implements TokenStore {
	...
	private static final boolean springDataRedis_2_0 = ClassUtils
			.isPresent(
	...
	private OAuth2AccessToken deserializeAccessToken(byte[] bytes) {
		return serializationStrategy
				.deserialize(bytes, OAuth2AccessToken.class);
	}
	...
	public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
		...
		if (accessToken != null) {
			OAuth2Authentication storedAuthentication = readAuthentication(accessToken
					.getValue());
			if ((storedAuthentication == null || !key
					.equals(authenticationKeyGenerator
							.extractKey(storedAuthentication)))) {
	...
	public OAuth2Authentication readAuthentication(OAuth2AccessToken token) {
		...
		if (isRenew && auth2Authentication != null) {
			...
			if (checkRenewClientId(clientAuth.getClientId())) {
				// 获取过期时长
				ClientDetails clientDetails = getClientDetails(clientAuth
						.getClientId());
				int validitySeconds = Objects.nonNull(clientDetails) ? clientDetails
						.getAccessTokenValiditySeconds()
						: SecurityConstants.ACCESS_TOKEN_VALIDITY_SECONDS;
				if (validitySeconds > 0) {
					double expiresRatio = token.getExpiresIn()
							/ (double) validitySeconds;
					// 判断是否需要续签，当前剩余时间小于过期时长的50%则续签
					if (expiresRatio <= securityProperties.getAuth().getRenew()
							.getTimeRatio()) {
						// 更新AccessToken过期时间
						DefaultOAuth2AccessToken oAuth2AccessToken = (DefaultOAuth2AccessToken) token;
						oAuth2AccessToken.setExpiration(new Date(System
								.currentTimeMillis()
								+ (validitySeconds * 1000L)));
						storeAccessToken(oAuth2AccessToken,
								auth2Authentication, true, clientDetails);
					}
				}
			}
		}
	...
	private boolean checkRenewClientId(String clientId) {
		boolean result = true;
		// 白名单
		List<String> includeClientIds = securityProperties.getAuth().getRenew()
				.getIncludeClientIds();
		// 黑名单
		List<String> exclusiveClientIds = securityProperties.getAuth()
				.getRenew().getExclusiveClientIds();
	...
	private int getAccessTokenValiditySeconds(String clientId) {
		...
		byte[] bytes;
		try {
			bytes = conn.get(serializeKey(SecurityConstants.CACHE_CLIENT_KEY
					+ ":" + clientId));
		} finally {
			conn.close();
		}
	...
	public OAuth2Authentication readAuthentication(String token) {
		...
		RedisConnection conn = getConnection();
		try {
			bytes = conn.get(serializeKey(SecurityConstants.REDIS_TOKEN_AUTH
					+ token));
		} finally {
			conn.close();
		}
	...
	public void storeAccessToken(OAuth2AccessToken token,
		...
		storeAccessToken(token, authentication, false, null);
	}
	 ...
	 *            是否续签
	 */
	private void storeAccessToken(OAuth2AccessToken token,
			OAuth2Authentication authentication, boolean isRenew,
			ClientDetails clientDetails) {
		Object sysUser = authentication.getPrincipal();
		String username = "";
		if (Objects.nonNull(sysUser) && sysUser instanceof SysUser) {
			username = ((SysUser) sysUser).getUsername();
		}
		byte[] accessBakKey = serializeKey(ACCESS_BAK + token.getValue());
		byte[] serializedAccessToken = serialize(token);
		byte[] serializedAuth = serialize(authentication);
		byte[] accessKey = serializeKey(ACCESS + token.getValue());
		byte[] loginUser = serializeKey(SecurityConstants.LOGIN_KEY
				.concat(username));
		byte[] authKey = serializeKey(SecurityConstants.REDIS_TOKEN_AUTH
				+ token.getValue());
		byte[] authToAccessKey = serializeKey(AUTH_TO_ACCESS
				+ authenticationKeyGenerator.extractKey(authentication));
		byte[] approvalKey = serializeKey(SecurityConstants.REDIS_UNAME_TO_ACCESS
				+ getApprovalKey(authentication));
		byte[] clientId = serializeKey(SecurityConstants.REDIS_CLIENT_ID_TO_ACCESS
		...
		try {
			...
			if (springDataRedis_2_0) {
				try {
					this.redisConnectionSet_2_0.invoke(conn, accessKey,
							serializedAccessToken);
					this.redisConnectionSet_2_0.invoke(conn, accessBakKey,
							serializedAccessToken);
					this.redisConnectionSet_2_0.invoke(conn, authKey,
							serializedAuth);
					this.redisConnectionSet_2_0.invoke(conn, authToAccessKey,
							serializedAccessToken);
				} catch (Exception ex) {
			...
			if (refreshToken != null && refreshToken.getValue() != null) {
				...
				byte[] refreshToAccessKey = serializeKey(REFRESH_TO_ACCESS
						+ token.getRefreshToken().getValue());
				byte[] accessToRefreshKey = serializeKey(ACCESS_TO_REFRESH
						+ token.getValue());
				if (springDataRedis_2_0) {
					try {
						this.redisConnectionSet_2_0.invoke(conn,
				...
				int seconds = Objects.isNull(clientDetails) ? token
						.getExpiresIn() : clientDetails
						.getRefreshTokenValiditySeconds();
				expireRefreshToken(refreshToken, conn, refreshToAccessKey,
						accessToRefreshKey, seconds);
			}
			conn.closePipeline();
		} finally {
			conn.close();
	...
	private static String getApprovalKey(OAuth2Authentication authentication) {
		...
		return getApprovalKey(authentication.getOAuth2Request().getClientId(),
				userName);
	}
	...
	public void removeAccessToken(String tokenValue) {
		...
		try {
			...
			if (authentication != null) {
				...
				byte[] unameKey = serializeKey(SecurityConstants.REDIS_UNAME_TO_ACCESS
						+ getApprovalKey(authentication));
				byte[] clientId = serializeKey(SecurityConstants.REDIS_CLIENT_ID_TO_ACCESS
						+ authentication.getOAuth2Request().getClientId());
				conn.openPipeline();
	...
	public void storeRefreshToken(OAuth2RefreshToken refreshToken,
		...
		byte[] refreshAuthKey = serializeKey(REFRESH_AUTH
		...
		try {
			...
			if (springDataRedis_2_0) {
				try {
					this.redisConnectionSet_2_0.invoke(conn, refreshKey,
							serializedRefreshToken);
					this.redisConnectionSet_2_0.invoke(conn, refreshAuthKey,
	...
	private void expireRefreshToken(OAuth2RefreshToken refreshToken,
			RedisConnection conn, byte[] refreshKey, byte[] refreshAuthKey,
			int... seconds) {
		if (refreshToken instanceof ExpiringOAuth2RefreshToken) {
			ExpiringOAuth2RefreshToken expiringRefreshToken = (ExpiringOAuth2RefreshToken) refreshToken;
			Date expiration = expiringRefreshToken.getExpiration();
			if (expiration != null) {
				if (Objects.nonNull(seconds) && seconds.length != 0) {
					conn.expire(refreshKey, seconds[0]);
					conn.expire(refreshAuthKey, seconds[0]);
				} else {
					int expireSeconds = Long
							.valueOf(
									(expiration.getTime() - System
	...
	public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
		byte[] key = serializeKey(SecurityConstants.REDIS_CLIENT_ID_TO_ACCESS
				+ clientId);
		List<byte[]> byteList;
		RedisConnection conn = getConnection();
		try {
	...
	private Collection<OAuth2AccessToken> getTokenCollections(
			List<byte[]> byteList) {
