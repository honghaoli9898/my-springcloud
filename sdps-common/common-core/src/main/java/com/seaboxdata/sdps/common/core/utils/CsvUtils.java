...
public class CsvUtils {
	 ...
	 *            输出到的流
	 * @param res
	 *            HttpServletResponse（可选），如果指定了就会添加文件下载的头部
	 * @param fileName
	 *            可选，文件名，用户下载的文件名，传入 res 有效
	 * @param <T>
	 * @throws IOException
	 */
	public static <T extends Object> void writeCsv(List<T> items,
			OutputStream os, HttpServletResponse res, String fileName)
	...
	private static void setHttpHeader(HttpServletResponse res, String fileName) {
		fileName = StringUtils.isEmpty(fileName) ? (generateRandomFileName() + ".csv")
				: fileName;
		// res.setHeader("content-type", "application/octet-stream");
		res.setHeader("content-type", "application/octet-stream; charset="
				+ charset);
	...
	// 初始化要输出的CSV字段
	private static <T extends Object> List<FieldData> initCsvFields(
			List<T> items, List<String> titles) {
		...
		for (int i = 0; i < fields.length; i++) {
			Alias item = fields[i].getAnnotation(Alias.class);
			if (item != null) {
				csvFields.add(new FieldData(fields[i]));
				if (item == null)
	...
	// 写入数据
	private static <T extends Object> void writeData(List<T> items,
			List<String> titles, List<FieldData> csvFields, OutputStream os)
			throws IOException, NoSuchMethodException,
			InvocationTargetException, IllegalAccessException,
			JsonProcessingException {
		// 写入标题
		String text = stringArrayToCsvLine(titles.toArray(new String[titles
				.size()])) + "\n";
	...
	private static List<Method> fieldToMethods(List<FieldData> csvFields,
			Object item) throws NoSuchMethodException {
	...
	private static String itemToString(Object item,
			@NotNull List<Method> methods, List<FieldData> csvFields)
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (values[i] == null || values[i].isEmpty())
				continue;
			if (!values[i].isEmpty()
					&& (values[i].startsWith("{") || values[i].startsWith("[")))
