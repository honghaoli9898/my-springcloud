package com.seaboxdata.sdps.oss.model;
 ...
 * Hadoop 操作类
 * 
 * @author Ruison on 2019/7/4 - 10:26
 */
@Slf4j
@AllArgsConstructor
public class HadoopClient {
	 ...
	 *            fs的目标路径
	 */
	public void copyFileToHDFS(boolean delSrc, boolean overwrite,
			String srcFile, String destPath) {
	...
	public void download(String path, String fileName, OutputStream outputStream)
		...
		@Cleanup
		InputStream is = fs.open(new Path(path + fileName));
	...
	public List<Map<String, String>> getFileList(String path) {
		...
		try {
			// 递归找到所有文件
			RemoteIterator<LocatedFileStatus> filesList = fs.listFiles(
					new Path(path), true);
	...
	public boolean renameFile(String oldName, String newName) {
		log.info("【文件或文件夹重命名】 开始重命名, 旧文件或旧文件夹名称: {}, 新文件或新文件夹名称: {} ", oldName,
				newName);
		boolean isOk = false;
	...
	public void copyFile(String sourcePath, String targetPath) {
		...
		try {
			...
			IOUtils.copyBytes(inputStream, outputStream, 1024 * 1024 * 64,
					false);
			IOUtils.cleanupWithLogger(null,inputStream, outputStream);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
