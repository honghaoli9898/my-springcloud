...
public class Cas20ServiceTicketValidator extends
	...
	private HttpURLConnectionFactory urlConnectionFactory = new HttpsURLConnectionFactory(
			hostnameVerifier, new Properties());
	...
	public Cas20ServiceTicketValidator(final String casServerUrlPrefix) {
		...
		this.proxyRetriever = new Cas20ProxyRetriever(casServerUrlPrefix,
				getEncoding(), urlConnectionFactory);
	 ...
	 */
	@Override
	protected void populateUrlAttributeMap(
	...
	protected String retrieveProxyGrantingTicket(final String response) {
		final List<String> values = XmlUtils.getTextForElements(response,
				PGT_ATTRIBUTE);
	...
	protected String retrieveProxyGrantingTicketViaEncryption(
			final String encryptedPgt) {
		if (this.privateKey != null) {
			try {
				final Cipher cipher = Cipher.getInstance(privateKey
						.getAlgorithm());
	...
	protected String parseAuthenticationFailureFromResponse(
	 ...
	 * &lt;cas:attributes&gt
	 * ;&lt;cas:attribute1&gt;value&lt;/cas:attribute1&gt;&lt
	 * ;cas:attribute2&gt;value&lt;/cas:attribute2&gt;&lt;/cas:attributes&gt;
	 * <p>
	 *
