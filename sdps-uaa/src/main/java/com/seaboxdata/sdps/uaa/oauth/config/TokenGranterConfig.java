...
import com.seaboxdata.sdps.common.captcha.service.CaptchaService;
import com.seaboxdata.sdps.common.oauth2.properties.SecurityProperties;
import com.seaboxdata.sdps.uaa.oauth.granter.CasTicketTokenGranter;
import com.seaboxdata.sdps.uaa.oauth.granter.MobilePwdGranter;
import com.seaboxdata.sdps.uaa.oauth.granter.OpenIdGranter;
import com.seaboxdata.sdps.uaa.oauth.granter.PwdImgCodeGranter;
import com.seaboxdata.sdps.uaa.oauth.service.IValidateCodeService;
import com.seaboxdata.sdps.uaa.oauth.service.impl.CustomTokenServices;
import com.seaboxdata.sdps.uaa.oauth.service.impl.UserDetailServiceFactory;
import com.seaboxdata.sdps.uaa.oauth.service.impl.UserDetailsByNameServiceFactoryWrapper;
...
public class TokenGranterConfig {
				...
				public OAuth2AccessToken grant(String grantType,
					...
					if (delegate == null) {
						delegate = new CompositeTokenGranter(
								getAllTokenGranters(tokenServices));
					}
	 ...
	 * 所有授权模式：默认的5种模式 + 自定义的模式
	 */
	protected List<TokenGranter> getAllTokenGranters(
		...
		if (authenticationManager != null) {
			// 添加密码加图形验证码模式
			tokenGranters.add(new PwdImgCodeGranter(authenticationManager,
					tokenServices, clientDetailsService, requestFactory,
					validateCodeService, captchaService));
			// 添加openId模式
			tokenGranters.add(new OpenIdGranter(authenticationManager,
					tokenServices, clientDetailsService, requestFactory));
			// 添加手机号加密码授权模式
			tokenGranters.add(new MobilePwdGranter(authenticationManager,
					tokenServices, clientDetailsService, requestFactory));
			//添加cas模式
			tokenGranters.add(new CasTicketTokenGranter(authenticationManager,
					tokenServices, clientDetailsService, requestFactory));
		}
	...
	private List<TokenGranter> getDefaultTokenGranters(
		...
		// 添加授权码模式
		tokenGranters
				.add(new AuthorizationCodeTokenGranter(tokenServices,
	...
	private void addUserDetailsService(DefaultTokenServices tokenServices) {
		if (this.userDetailsServiceFactory != null) {
			...
			provider.setPreAuthenticatedUserDetailsService(new UserDetailsByNameServiceFactoryWrapper<>(
