...
object OriginTransformForward {
  def main(args: Array[String]): Unit = {
    ...
    val spark: SparkSession = SparkSession.builder().appName("OriginDataTransformForward").getOrCreate()
    ...
    val dataDF: DataFrame = sc.textFile(originFilePath).map(_.split("\t")).map(line => {
      ...
      OutputDefine2(timeStamp,today, hour, minute, userID, topic, resultRank, clickRank, url)
    }).toDF()

    dataDF.write.format("delta").option("mergeSchema",true).mode("Overwrite").save(outputPath)

    spark.close()
  }
}

case class OutputDefine2(timeStamp: Long,date:String, hour:Int, minute:Int, userId:Int, topic:String, resultRank:Int, clickRank:Int, url:String)